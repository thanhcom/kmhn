            ...
            steps {
                sshagent(['ssh-remote']) {
                    sh '''
                        ...
                        ssh -o StrictHostKeyChecking=no docker exec Tomcat.11 rm -rf /usr/local/tomcat/webapps/*.war
                    '''
                }
            }
        }
            ...
            steps {
                sshagent(['ssh-remote']) {
                    sh '''
                        ...
                        ssh -o StrictHostKeyChecking=no docker exec Tomcat.11 rm -rf /usr/local/tomcat/webapps/kmhn
                    '''
                }
            }
        }
